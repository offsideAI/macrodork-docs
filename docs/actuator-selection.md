...
> 🔌 For the servo-bus protocol comparison (including Feetech STS/SCS and HD-1910 specs) see [Hardware Teardown](hardware-teardown.md).
...
**The upstream design actually feeds them 6.6 – 8.2 V.** Three independent points in the source agree:
...
**value recommended in the ROBOTIS datasheet**, not what the upstream design actually wires up.
...
**So upstream runs the XL330 continuously above its 6.0 V rated maximum, up to about 8.2 V.**
...
- ⚠️ **Lifetime and heat are the price.** Over-voltage stresses windings and MOSFETs. Upstream has
  published nothing on this, so **the following is inference**: it may relate to the servo heating
  users report, and to upstream starting to consider selling repair kits (see [Community Intelligence](community-intelligence.md)).
...
Upstream has never named the sub-model publicly - the source carries only
...
|---|---|---|---|
| **Macrodork** | XL330 | **737.2 g** | 25 cm |
| **Open Duck Mini v2** | STS3215 | **2107.1 g** | 42 cm |

**2.86×.**

> 💡 **Open Duck Mini v2 *is* the STS3215 answer.** Same engineer (Antoine Pirrone,
> upstream R&D), same bipedal-duck architecture - swap the servo and the robot can only be
...
| **Feetech SCS0009** | **11 g** | - | 0.23 N·m<br>(2.3 kg·cm) | 6 V | 10-bit | 1:416 metal gears |
...
> **This table uses vendor stall-torque figures throughout** (checked against each
...
> Everything else is vendor-published data. **Prices vary enormously by region and are not
> listed here** - the same STS3032 retails imported in India at about ₹3,451, and at an
> entirely different price level in China.
...
| Bus physical layer | single-wire half-duplex TTL | **single-wire half-duplex TTL (1-Wire)** | ✅ |
...
| **Voltage** | ~7.4 V (2S) in practice - rated 3.7–6.0 V, **run at 6.6–8.2 V** | **12.6 V (3S)** | ⚠️ **incompatible** |
| **Bus capacity** | 253 addresses | **15 (0–14; 15 is broadcast)** | ⚠️ |
| Torque | **0.96 N·m** | **unclear** | ⚠️ |

> Dimensions, connector, protocol and control law come from the official
> [J288/S288 brushless digital servo user manual](https://www.unitree.com/images/%E6%97%A0%E5%88%B7%E6%95%B0%E5%AD%97%E8%88%B5%E6%9C%BAJ288S288%E4%BD%BF%E7%94%A8%E6%89%8B%E5%86%8C.pdf)
> (11 pages, read in full); mass, voltage and encoder resolution from the
...
Upstream's `limp_fall` buys compliance by dropping kp from 200 to 50; with an S288 you
...
rotor torque/speed/position, and a 22-bit fault code (over-current, over-voltage,
under-voltage, chip/MOSFET/case/winding over-temperature, errors on each of the three
encoders, and so on). Far richer than the XL330's.

#### Three hard constraints

**① Voltage: 12.6 V vs 7.4 V**
This robot is 2S (NP-F550); the S288 wants **3S**. Battery, power distribution and the HAT
all have to be redesigned.

**② The bus holds only 15 servos**
The address space is **0–14** (15 is broadcast). The robot has exactly 15 servos - **the
...
> This agrees with the recommendation in [Hardware Teardown](hardware-teardown.md):
...
| Those figures are **output-side** | 0.2 N·m | ❌ one fifth of the XL330's (0.96) - not a drop-in |
...
| ❌ **Protocol** | Unitree's own 20-byte command / 26-byte reply with CRC32 at 6 Mbps, nothing like Dynamixel Protocol V2; `rustypot` does not support it, so the communication layer must be rewritten |
| ❌ **Policies** | All nine official ONNX policies become invalid - both the torque characteristics and the control mode change |
...
**Two problems - and note that this document previously overstated both:**

1. **Torque is about 85 %, not 46 %** - on a same-voltage basis, 0.44 N·m @6 V vs the
   XL330's 0.52 N·m @5 V. The earlier "46 %" compared 0.44 against the simulation force
   limit 0.96; the two are not comparable.
2. **The voltage ceiling is 6 V - but the XL330's ceiling is also 6 V.**
   The difference is not in the servo specs; it is that **upstream chose to run over-voltage**
   (see the section above). To copy the design with an STS3032 you either over-volt it
   the same way (at your own risk) or add a regulator stage down to 5 V.
...
response delay - training would need per-group actuator models, which adds considerable
complexity.

### A telling data point: upstream uses Feetech themselves

**The SCS0009 is used in upstream's own
[Amazing Hand](https://github.com/pollen-robotics/AmazingHand) and HopeJR.**

They are not unaware of the cheap option - **these legs genuinely need that
torque.** 0.23 N·m is enough for a finger joint and not enough for a biped's knee.

### Where the gap is

```
Wanted:  18-20 g  .  >=0.5 N.m at rated voltage  .  tolerates direct 2S supply or carries its own regulator  .  bus daisy chain  .  integrated encoder + driver
Today:   Robotis XL330     meets all of it, but must be over-volted to 6.6-8.2 V to do so
         Unitree S288      meets mass/size/ratio/bus; the 12.6 V (3S) rating is the only obstacle
         Feetech STS3032   meets mass/size, torque ~85 %, shares the XL330's 6 V ceiling
```

**The gap is being squeezed - the Unitree S288 has closed most of it.** What still blocks
it is the voltage class (3S vs 2S) and the unresolved torque labelling. If Unitree ships a
2S variant, or the torque is confirmed at the 0.9 N·m level, the slot is filled.

Feetech's line is heading the same way - **SCS0009 (11 g) → STS3032 (20 g) is a clear
"smaller and bus-connected" trajectory**; the housing is already there, what is missing is
raising the ratio or the voltage class to lift the torque into the 0.9 N·m range.

Two market signals reinforce this:

- The **LeRobot ecosystem** (Hugging Face) has already made the STS3215 a de facto standard for arms
- **The upstream robot passed $1 M in sales within seven hours of launch**, proving real demand at the 18–20 g scale
...
> demonstrated - it is plausible that this slot gets filled.

**If such a part does appear**, this repository becomes more valuable, not less: the
mechanical geometry, the assembly relationships, the electronics design and the actuator
parameters are all already understood, so adapting is a matter of days rather than starting
from zero.
...
| **Switch to the Unitree S288** | Mechanically almost a match (19.5 g / 34×20×23 / 288.35:1 / M2), and brings joint force control plus an output-side encoder. Needs a 3S pack, a rewritten communication layer and retrained policies - **and the torque spec is ambiguous and must be confirmed first** - see the dedicated section above |
...
**Within this weight class, only Robotis currently offers enough torque.** The Feetech
STS3032 is smaller and about the same mass, but delivers about **85 %** of the torque
(0.44 N·m @6 V vs the XL330's 0.52 N·m @5 V, same-voltage basis) with a 6 V ceiling; the
SCS0009 is lighter (11 g) but only 0.23 N·m. See the side-by-side comparison in the
previous section.
