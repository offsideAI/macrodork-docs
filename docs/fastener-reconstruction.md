...
> 🛒 Sourcing links for screw kits, heat-set inserts and insert tips, and thread locker: [mechanical sourcing list](mechanical-sourcing-list.md).

There is no mechanical BOM upstream and no screw list (the HAT board has an electronics BOM, but it covers no structural parts). Everything below was **recovered from the geometry of the 47 STL meshes.**
...
6. Measure angular coverage and keep only complete circular holes (≥300°)
...
> control; the fifteenth drives the beak/jaw (in the `passive_*` linkage, outside the action
> space). This matches the "15 servos" in the official README.
...
- M2-class holes (Ø1.9–2.5 mm, ≥300° wrap) across the assembly, **weighted by usage: 237**,
  ...
  and about 156 on structural parts

  > ⚠️ **Correction (2026-09-04)**: earlier versions said "213", which does not reconcile. Two
  > causes: the earlier count was **not weighted by how often each part is used** (`leg` is
  > actually ×4, `hip_l` / `yaw2roll` and others ×2), and it **included meshes no MJCF references**.
  > The diameter and depth tables below are **unweighted** (each STL counted once) - the two
  > tables cannot simply be added together.
...
| M2 heat-set inserts | 60 | recommended over tapping printed parts directly |
